...
concateURL = str("https://api.mysportsfeeds.com/v2.1/pull/nhl/2023-2024-regular/date/"+todayDate+"/player_gamelogs.json")
# store private authentication key from environment files
# key = str(os.environ.get('fantasyPoolApiKey'))
key = 'e5fce1f2-06aa-4e45-8336-3c1434'
    ...
    try:
        response = requests.get(
            ...
            params={
                ...
                "team":{'tor,col,flo,dal,car,bos,edm,tbl,min,nyr,lak,njd,nyi,sea,vgk,wpj'}
            },
...
def formatStats():
    ...
    sendTodayStatDF['Team'].replace({'FLO':'FLA','WPJ':'WPG'}, inplace = True)
    ...
    sendStatDF.to_csv('playerStatsTotal.csv', index = False)
    
    return sendStatDF

def buildAgg():
    print(f"Last update at: {str(datetime.datetime.now())[:19]}")
    ...
    aggStats.to_csv('playerStatsAgg.csv', index = False)
